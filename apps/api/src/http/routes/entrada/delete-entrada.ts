...
export async function deleteEntrada(app: FastifyInstance){
    app.withTypeProvider<ZodTypeProvider>().register(auth).delete('/entrada/:id', {
            schema:{
                tags: ['Entrada'],
        ...
        async(request, reply) => {
            ...
            const saidaFound = await prisma.entrada_info.findFirst({
                where: {
                    id: id,
                    id_ativo: true,
                    id_info_ativo: true,
            ...
            const result = await prisma.$transaction(async (tx) => {
                const deletedInfo = await prisma.entrada_info.update({
                    where:{
                        id
                    },                    
                    data: {
                ...
                try{
                    ...
                    const notificacao = await prisma.notificacao.create({
                        data: {
                            descricao: `O usuário ${userFromId.usuario.nome} deletou uma entrada`,
                            dthr_cadastro: new Date(),
                            titulo: 'Saída registrada',
                            visto: false,
                            id_saida_info: deletedInfo.id,
