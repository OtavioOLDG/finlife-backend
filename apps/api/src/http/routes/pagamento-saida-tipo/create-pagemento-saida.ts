...
export async function createOutcome(app: FastifyInstance){
    app.withTypeProvider<ZodTypeProvider>().register(auth).post('/saida-tipo', {
            schema:{
                tags: ['Pagamento Saída Tipo'],
                summary: 'Cadastra um novo tipo de saída',
                security: [{bearerAuth: []}],
                body: z.object({
                    nome: z.string(),
