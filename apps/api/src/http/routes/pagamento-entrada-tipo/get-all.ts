...
export async function getAllTiposEntrada(app: FastifyInstance){
    app.withTypeProvider<ZodTypeProvider>().register(auth).get('/entrada-tipo/todos', {
            schema:{
                tags: ['Pagamento Entrada Tipo'],
                summary: 'Pega todos os tipos de entrada',
                security: [{bearerAuth: []}],
                response: {
                    201: z.object({
        ...
        async(request, reply) => {
            ...
            const tiposPublicos = await prisma.pagamento_entrada_tipo.findMany({
                where: {
                    publico: true
                }
            }) 
            ...
            const tiposPrivados = await prisma.pagamento_entrada_tipo.findMany({
                where: {
                    publico: false,
                    id_ativo: true,
